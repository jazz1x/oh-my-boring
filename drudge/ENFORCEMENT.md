...
② Gate         pre-commit fast hook · make guard · pre-deploy(eval-gate.sh)   ← blocks commit/deploy
...
|---|---|---|---|
| **§0 Rust official** (fmt · clippy · RFC430 naming · C-CONV `as_/to_/into_` · C-COMMON-TRAITS) | **fmt/clippy (machine)** + some naming/trait review | `drudge/Cargo.toml:46-69` · `scripts/guard.sh` | Shared contract — no need to memorize. *Not my philosophy* |
...
| no unwrap/expect/panic/todo/unimplemented/unreachable (§B flow) | **clippy restriction (deny)** | `drudge/Cargo.toml:64-69` | Cargo `[lints]` SSOT |
| `unsafe` forbidden | **`unsafe_code="forbid"`** | `drudge/Cargo.toml:48` | compiler |
...
| pedantic idioms (semicolon · needless, etc.) | **clippy pedantic (deny)** | `drudge/Cargo.toml:51-52` | nursery excluded (time bomb) |
| formatting + adapter/shell regressions | **`make guard`**: rustfmt, clippy, Rust tests, Python compile/unit tests, shell guardrails, data-steward dry-run | `Makefile` · `scripts/guard.sh` · `.pre-commit-config.yaml` | Stack-free structural gate; pre-commit is a faster staged subset |
...
| **D6** No-panic / ROP / Layer 1>2>3 | `drudge/PHILOSOPHY.md` · `drudge/Cargo.toml:46-69` |
...
## Gates
- **pre-commit fast hook** = `.pre-commit-config.yaml`: whitespace/YAML/TOML/secret checks plus staged Rust/Python syntax gates. Install: `pre-commit install`.
- **full local structural gate** = `make guard` (stack-free): `scripts/guard.sh` plus data-steward dry-run.
